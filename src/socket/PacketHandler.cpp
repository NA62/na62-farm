...
l0::MEP* mep_L1 = new l0::MEP(L1Data + sizeof(UDP_HDR), L1BlockLength, { L1Data,
		L1BlockLength });
...
BytesReceivedBySourceNum_[sourceNum].fetch_add(L1BlockLength + sizeof(UDP_HDR),
...
l0::MEP* mep_L2 = new l0::MEP(L2Data + sizeof(UDP_HDR), L2DataLength, { L2Data,
		L2DataLength });
uint sourceNum = SourceIDManager::sourceIDToNum(mep_L2->getSourceID());

MEPsReceivedBySourceNum_[sourceNum].fetch_add(1, std::memory_order_relaxed);
BytesReceivedBySourceNum_[sourceNum].fetch_add(L2BlockLength + sizeof(UDP_HDR),
		std::memory_order_relaxed);
...
void PacketHandler::processDestPortCREAM(const char* UDPPayload,
	uint_fast16_t UdpDataLength, DataContainer container) {
cream::LkrFragment* fragment = new cream::LkrFragment(UDPPayload, UdpDataLength,
		container);
